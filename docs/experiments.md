...
...
```
Since we work with features and classifications independently, we can create contiguous arrays for each of them, which will improve locality and cache hits. So we now have a `Structure of Arrays` instead of an `Array of Structures`:
...
#### Replacing parameters with constants

The algorithm functions are generic, in the sense that they work with any number of features and points. However, we know that our dataset has a fixed number of features and points: we'll specialize the functions to work with those values, which might allow the compiler to optimize them further.

Our experiments show very marginal improvements in execution time.

#### Replacing `plurality_voting` histogram

The `plurality_voting` function uses a histogram to count the number of occurrences of each classification. We can replace this histogram with a fixed number of variables, one for each classification, and then compare them directly. This works only for K=3, since K=20 would require 20 variables and the number of comparisons would explode.

With this specialized function, we get a small reduction in execution time.
...
Time: 0.1259 s
  ...
  %   cumulative   self              self     total           
 time   seconds   seconds    calls  us/call  us/call  name    
 88.46      0.23     0.23     1082   212.57   240.30  get_k_NN_soa
 11.54      0.26     0.03     1082    27.73    27.73  select_k_nearest_specific
  0.00      0.26     0.00     1082     0.00     0.00  copy_k_nearest_specific
  0.00      0.26     0.00     1082     0.00   240.30  knn_classifyinstance_soa
  0.00      0.26     0.00     1082     0.00     0.00  minmax_normalize_point
  0.00      0.26     0.00     1082     0.00     0.00  plurality_voting
  0.00      0.26     0.00        1     0.00     0.00  extract_soa
  0.00      0.26     0.00        1     0.00     0.00  minmax
  0.00      0.26     0.00        1     0.00     0.00  minmax_normalize
  0.00      0.26     0.00        1     0.00     0.00  timer_destroy
  0.00      0.26     0.00        1     0.00     0.00  timer_get_s
  0.00      0.26     0.00        1     0.00     0.00  timer_init
  0.00      0.26     0.00        1     0.00     0.00  timer_start
  0.00      0.26     0.00        1     0.00     0.00  timer_stop
  0.00      0.26     0.00        1     0.00     0.00  verify_results
```
