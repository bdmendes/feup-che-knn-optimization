...
# 3 "src/utils.c" 2
# 1 "/tools/Xilinx/Vitis_HLS/2023.2/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/float.h" 1 3
# 4 "src/utils.c" 2
...
int verify_results(int num_new_points, const Point *new_points, const char *key);
...
void minmax_normalize_point(double *min, double *max, Point *point, int num_features);
# 7 "src/utils.c" 2
...
int verify_results(int num_new_points, const Point *new_points, const char *key)
{
    ...
    VITIS_LOOP_30_1: for (int i = 0; i < num_new_points; ++i)
    {
...
static int rand_int(int min, int max)
{
    ...
    int number = (int)(min + rand() / (2147483647 / (max - min + 1) + 1));
    return number;
}
...
static double rand_double(double min, double max)
{
    ...
    double number = (double)(min + rand() / (2147483647 / (max - min + 1) + 1));
    return number;
}
...
void initialize_known_points(int num_points, Point *known_points, int num_classes, int num_features)
{
    ...
    VITIS_LOOP_116_1: for (int i = 0; i < num_points; i++)
    {
        VITIS_LOOP_118_2: for (int j = 0; j < num_features; j++)
        {
...
void initialize_new_points(int num_new_points, Point *new_points, int num_features)
{
    ...
    VITIS_LOOP_132_1: for (int i = 0; i < num_new_points; i++)
    {
        VITIS_LOOP_134_2: for (int j = 0; j < num_features; j++)
        {
...
void show_point(Point point, int num_features)
{
    ...
    VITIS_LOOP_148_1: for (int j = 0; j < num_features; j++)
    {
        if (j == 0)
...
void minmax(double *min, double *max, int num_points, Point *known_points, int num_features)
{
    ...
    VITIS_LOOP_165_1: for (int j = 0; j < num_features; j++)
    {
    ...
    VITIS_LOOP_172_2: for (int i = 0; i < num_points; i++)
    {
        VITIS_LOOP_174_3: for (int j = 0; j < num_features; j++)
        {
...
void minmax_normalize(double *min, double *max, int num_points, Point *points, int num_features)
{
    ...
    VITIS_LOOP_205_1: for (int i = 0; i < num_points; i++)
    {
        VITIS_LOOP_207_2: for (int j = 0; j < num_features; j++)
        {

            double nfeature = (double)((points[i].features[j] - min[j]) / (max[j] - min[j]));


            if (__builtin_isnan (nfeature))
                nfeature = (double)0.0;
            else if (__builtin_isinf_sign (nfeature))
                nfeature = (double)1.0;
...
void minmax_normalize_point(double *min, double *max, Point *point, int num_features)
{
    ...
    VITIS_LOOP_234_1: for (int j = 0; j < num_features; j++)
    {

        double nfeature = (double)((point->features[j] - min[j]) / (max[j] - min[j]));
