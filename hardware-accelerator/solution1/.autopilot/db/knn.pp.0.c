...
char plurality_voting(BestPoint *best_points);

char knn_classifyinstance(Point new_point, Point *known_points);
# 28 "src/knn.c" 2
...
void copy_k_nearest_specific(BestPoint *dist_points, BestPoint *best_points) {
    ...
    memcpy(best_points, dist_points, 20 * sizeof(BestPoint));
}
...
void select_k_nearest_specific(BestPoint *dist_points) {
    ...
    VITIS_LOOP_52_1: for (int i = 0; i < 20; i++) {
        min_distance = dist_points[i].distance;
        index = i;
        VITIS_LOOP_55_2: for (int j = i + 1; j < 4336; j++) {
...
void copy_k_nearest(BestPoint *dist_points, BestPoint *best_points) {
    ...
    VITIS_LOOP_80_1: for (int i = 0; i < 20; i++) {
...
void select_k_nearest(BestPoint *dist_points) {
    ...
    VITIS_LOOP_101_1: for (int i = 0; i < 20; i++) {
        min_distance = dist_points[i].distance;
        index = i;
        VITIS_LOOP_104_2: for (int j = i + 1; j < 4336; j++) {
            if (dist_points[j].distance < min_distance) {
                min_distance = dist_points[j].distance;
                index = j;
            }
...
void get_k_NN(Point new_point, Point *known_points, BestPoint *best_points) {
    ...
    BestPoint dist_points[4336];
# 152 "src/knn.c"
    VITIS_LOOP_152_1: for (int i = 0; i < 4336; i++) {
        double distance = (double)0.0;
# 163 "src/knn.c"
        VITIS_LOOP_163_2: for (int j = 0; j < 43; j++) {
            double diff = (double)new_point.features[j] -
                             (double)known_points[i].features[j];
            distance += diff * diff;
        }
...
# 225 "src/knn.c"
    ...
    VITIS_LOOP_230_1: for (int i = 0; i < 6; i++) {
        histogram[i] = 0;
    }
    ...
    VITIS_LOOP_239_2: for (int i = 0; i < 20; i++) {
    ...
    char max = 0;
    char classification_id = -1;
    VITIS_LOOP_253_3: for (int i = 0; i < 6; i++) {
        if (histogram[i] > max) {
            max = histogram[i];
...
char knn_classifyinstance(Point new_point, Point *known_points) {
    ...
    BestPoint best_points[20];
# 287 "src/knn.c"
    get_k_NN(new_point, known_points, best_points);
    char classID = plurality_voting(best_points);
