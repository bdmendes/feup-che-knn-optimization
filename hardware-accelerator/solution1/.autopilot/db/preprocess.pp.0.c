...
# 2 "<built-in>" 2
# 1 "src/preprocess.c" 2
# 1 "src/preprocess.h" 1
# 1 "src/params.h" 1
...
# 3 "src/preprocess.h" 2

Points extract_soa_inverted(Point *points,
                                    double *out_features, char *out_classifications);

Points extract_soa(Point *points,
                   double *out_features, char *out_classifications);
# 2 "src/preprocess.c" 2
...
# 3 "src/preprocess.c" 2
# 1 "/tools/Xilinx/Vitis_HLS/2023.2/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/stdbool.h" 1 3
# 4 "src/preprocess.c" 2

static void extract_features(Point *points,
                             double *out, _Bool inverted)
{
    VITIS_LOOP_8_1: for (int i = 0; i < 43; i++)
    {
        VITIS_LOOP_10_2: for (int j = 0; j < 4336; j++)
        {
            Point *point = &points[j];
            if (inverted)
            {
                out[i * 4336 + j] = point->features[i];
            }
            else
            {
                out[j * 43 + i] = point->features[i];
            }
        }
    }
}
...
static void extract_classifications(Point *points,
                                    char *out)
{
    VITIS_LOOP_28_1: for (int i = 0; i < 4336; i++)
    {
                                    ...
                                    double *out_features, char *out_classifications)
{
    ...
    extract_features(points, out_features, 1);
                   ...
                   double *out_features, char *out_classifications)
{
    Points points_soa;
    extract_classifications(points, out_classifications);
    extract_features(points, out_features, 0);
    points_soa.features = out_features;
    points_soa.classifications = out_classifications;
    return points_soa;
}
